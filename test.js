...
rl.on('line', (input) => {
  ...
  const rotate = (circleNumber, direction, times) => {
    ...
    if (direction === 1) {
      copyArr.map((i, index) => {
        times=times%M // times가 M보다 크면 안되므로 times를 M보다 작게
  ...
  // 인접한 숫자를 찾는 dfs 그래프 탐색
  let visitedForEachRotate = []; // dfs로 탐색할때마다 인접한 값들(x좌표 값)을 담아둠
  ...
  for (let i = 0; i < T; i++) {
    ...
    let totalOfOriginCircle = circleNumber
      ...
      .reduce((a, b) => a + b, 0);
    // if(totalOfOriginCircle===0){ // 이것도 가능은 함
    //   break
    // }
    ...
    for (let x = 0; x < N; x++) {
      for (let y = 0; y < M; y++) {
        if (circleNumber[x][y] !== 'x') {
          ...
          if (visitedForEachRotate.length === 1) { // 길이가 1인것은 인접한 다른 값이 없는 것이다
            circleNumber[x][y] = targetNumber; // x로 지정한 값을 다시 원래대로 돌려놓음
    ...
    // if (sumForEachRotate === 0 && totalOfOriginCircle!==0 )  가능
